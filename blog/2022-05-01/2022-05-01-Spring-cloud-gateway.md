---
title: Redis Request Rate Limiter를 만들어보자!
...
Sprging Cloud Gateway를 이용한 Custom Redis Rate Limiter 맛보기

<!--truncate-->

[모든 소스](https://github.com/sk1737030/til/tree/master/spring-cloud-gateway-late-limiter)는 요기서 확인가능합니다. :)

# Spring Cloud Gateway를 사용해서 API Limiter 구현하기
...
keyResolver를 세팅을 먼저 해준다. 요기서 return해주는 key를 가지고 뒤에서 구분을 하기 때문에 필수적으로 구현을 해야한다. 요기서는 getParameter로 userId가 들어온다는 가정하에 구현을 하였다.
...
<aside>
💡 참고로 RequestRateLimiter는 `[Token bucket` 알고리즘을](https://dev.to/satrobit/rate-limiting-using-the-token-bucket-algorithm-3cjh) 따른다.
...
이것도 `RedisRateLimiter`에 `getKeys`라는 메소드에서 확인 할 수 있는데 

![Untitled](spring-cloud-gateway/Untitled%204.png)

![Untitled](spring-cloud-gateway/Untitled%205.png)

사실 요기까지만해도 그냥 쓰기에도 나쁘지 않지만 조금 아쉽다. 사실 우리의 기획자나 비지니스 요구사항은 이렇게 간단하지 않다. 
...
냉혹한 비즈니스 세계에서는 이것보다도 더 많은 **Limiter**로써의 **역할**과 **책임** 을 요구하게 되는데, 기본적으로 제공하는 `RateLimiter`로는 작업이 어렵다. 
...
Request 요청시 위에 설정한 userAConfig에 맞게 Limter가 걸리게 되고, 
...
추가해도 좋을만한 옵션이 있는데, 만약 다양한 이유로 요청을 거절 하고 싶을 때  `deny-empty-key`라는 옵션이다. key가 없을시 *`FORBIDDEN` 응답으로 돌려주게된다.* 
...
limiter 설정 `requestedTokens` 1  `burstCapacity` 1  `replenishRate` 1 
...
- 정말 간단한 스크립트로 보면 알겠지만 10초동안 get요청을 한다는 스크립트이고, 결과로 200일 경우 성공으로 보겠다는 것이다.
...
이렇게 간단하게 `RateLimiter`를 구현 해보았는데, Gateway에 Limiter를 몰랐을 때는 구현을 어떻게 하지라며 고민을 했는데 (~~Watch와 Multi와 함께라면),~~  역시나 갓프링에 마침 잘 구현이 되어 있어서 쉽게 가져와서 사용을 하게 되었다. 요즘 그런 생각이든다. 때로는 직접 구현 할 필요도 있지만 이미 잘 만들어져서 책임을 다하는 객체가 있으면 잘 사용하는 게 더 좋겠다라고  

### 추가로 한 번 고민 해야 할 부분

Gateway에서 Request body를 읽어서 무언가를 처리 한 후에 다음 Filter로 넘겨 줄 때에는 고민을 해봐야하는데, 잘 알다시피 Servlet Request Body를 Filter에서 한 번 읽으면, 다시 뒤에서 body를 못 읽는다는 문제가 있다. 이 부분은 한 번 읽고 body를 캐싱 해서 다시 body를 넣어주는 추가 작업이 필요하다.
